...
if __name__ == '__main__':
    ...
    def match_and_draw(win):
        ...
        '''
        b_channel, g_channel, r_channel = cv2.split(vis)
        alpha_channel = np.ones((vis.shape[0], vis.shape[1]),dtype=np.uint8) #creating a dummy alpha channel image.
        alpha_channel.fill(255)
        img_RGBA = cv2.merge((b_channel, g_channel, r_channel, alpha_channel))
        '''
        send1.send('image', vis)
